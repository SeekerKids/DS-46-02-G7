...
public class AdsController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Menggunakan AdModel untuk mendapatkan iklan acak
        Ads adModel = new Ads();
        Ads randomAd = adModel.getRandomAd();

        // Menyimpan data dalam request scope
        if (randomAd != null) {
            request.setAttribute("adContent", randomAd.getContent());
        } else {
            request.setAttribute("adContent", null);
        }

        // Meneruskan ke JSP
        request.getRequestDispatcher("adView.jsp").forward(request, response);
    }
    @Override
